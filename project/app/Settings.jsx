// ─────────────────────────────────────────────────────────────
// AjustesSheet — gastos fijos editables + restablecer datos
// ─────────────────────────────────────────────────────────────
function AjustesSheet({ open, onClose, tx, jornal, onSetJornal, onEditTx, onManageCats, onReset }) {
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '8px 18px calc(env(safe-area-inset-bottom) + 24px)' }}>
            ...
            {editingJornal ?
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <input autoFocus value={jornalDraft} inputMode="decimal"
                  ...
                  style={{ width: 86, border: 'none', outline: 'none', background: '#00000008', borderRadius: 10, padding: '8px 10px', textAlign: 'right',
              ...
              <button onClick={() => setEditingJornal(true)} style={{ all: 'unset', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7 }}>
                <span style={{ fontFamily: 'var(--display)', fontSize: 18, fontWeight: 600, color: 'var(--ink)', fontVariantNumeric: 'tabular-nums' }}>{window.fmt(jornal || 0)}</span>
          ...
          <div style={{ borderRadius: 24, background: 'var(--hero)', color: '#F4EFE6', padding: '18px 20px', marginBottom: 18 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginTop: 5 }}>
              <span style={{ fontFamily: 'var(--display)', fontSize: 32, fontWeight: 600, letterSpacing: '-.02em', fontVariantNumeric: 'tabular-nums' }}>{window.fmtNum(totalFijos, 2)}</span>
          ...
          <div style={{ background: 'var(--paper)', borderRadius: 22, overflow: 'hidden', boxShadow: '0 1px 2px rgba(40,28,16,.04)' }}>
            {fixed.map((t, i) => {
              const cat = CAT[t.cat];
              return (
                <button key={t.id} onClick={() => onEditTx(t)} style={{ all: 'unset', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 13, padding: '13px 15px',
          ...
          <div style={{ background: 'var(--paper)', borderRadius: 22, overflow: 'hidden', boxShadow: '0 1px 2px rgba(40,28,16,.04)' }}>
            <button onClick={onManageCats} style={{ all: 'unset', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 13, padding: '15px 16px', width: '100%', boxSizing: 'border-box' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontFamily: 'var(--ui)', fontSize: 12, color: 'var(--ink-soft)', marginTop: 1 }}>Añadir, editar o borrar · {window.FIN.CATEGORIES.length} categorías</div>
              </div>
          ...
          <div style={{ background: 'var(--paper)', borderRadius: 22, overflow: 'hidden', boxShadow: '0 1px 2px rgba(40,28,16,.04)' }}>
            {!confirm ?
            <button onClick={() => setConfirm(true)} style={{ all: 'unset', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 13, padding: '15px 16px', width: '100%', boxSizing: 'border-box' }} data-comment-anchor="15fd5a9792-button-83-15">
...
window.AjustesSheet = AjustesSheet;