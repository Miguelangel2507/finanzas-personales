...
import { CatIcon, ProgressBar, Glyph, GLYPHS } from './components'
...
export function AddSheet({ open, onClose, onSave, onDelete, editTx, presetCat, cats }) {
  ...
  useEffect(() => {
    if (open) {
      if (editTx) {
        setAmount(String(editTx.amount).replace('.', ','))
        setCatId(editTx.cat)
        setDesc(editTx.desc)
        setDay(editTx.day)
      } else {
        setAmount('0')
        setCatId(presetCat || (CATEGORIES[0] ? CATEGORIES[0].id : 'ocio'))
        setDesc('')
        setDay(14)
      }
    }
  ...
  const cat = CAT[catId] || CATEGORIES[0]
  ...
  const save = () => {
    if (!canSave) return
    onSave({ id: editTx ? editTx.id : 't' + Date.now(), cat: catId, desc: desc || (cat ? cat.name : catId), amount: Math.round(numAmount * 100) / 100, day })
    onClose()
  }
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '0 0 8px' }}>
          ...
          <div style={{ textAlign: 'center', padding: '16px 0 10px' }}>
            ...
            <div style={{ marginTop: 4, fontFamily: 'var(--ui)', fontSize: 12.5, color: 'var(--ink-soft)' }}>
              {cat ? (cat.type === 'inversion' ? 'Inversión' : 'Gasto') : 'Gasto'} · {cat ? cat.name : catId}
            </div>
          </div>
          ...
          <div style={{ padding: '4px 18px 0' }}>
            ...
            <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
              {suggestions.map(s => (
                <button key={s} onClick={() => setDesc(s)} style={{
                  all: 'unset', cursor: 'pointer', fontFamily: 'var(--ui)', fontSize: 13, fontWeight: 500,
                  padding: '7px 13px', borderRadius: 99, background: desc === s ? 'var(--ink)' : 'var(--paper)', color: desc === s ? '#F4EFE6' : 'var(--ink)',
                  boxShadow: '0 1px 2px rgba(40,28,16,.05)', transition: 'all .15s'
                }}>{s}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ padding: '16px 18px 4px' }}>
            ...
            <div style={{ display: 'flex', gap: 7, overflowX: 'auto', scrollbarWidth: 'none', paddingBottom: 2 }}>
              {Array.from({ length: 14 }, (_, i) => 14 - i).map(dd => (
                <button key={dd} onClick={() => setDay(dd)} style={{
                  all: 'unset', cursor: 'pointer', flexShrink: 0, width: 42, height: 42, borderRadius: 13,
                  display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--display)', fontSize: 15, fontWeight: 600,
                  background: day === dd ? 'var(--accent)' : 'var(--paper)', color: day === dd ? '#fff' : 'var(--ink)', boxShadow: '0 1px 2px rgba(40,28,16,.05)'
                }}>{dd}</button>
              ))}
            </div>
          </div>
          ...
          {editTx && (
            <button onClick={() => { onDelete(editTx.id); onClose() }} style={{
              all: 'unset', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7,
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 9 }}>
            {['1', '2', '3', '4', '5', '6', '7', '8', '9', ',', '0', 'del'].map(k => (
              <button key={k} onClick={() => press(k)} style={{
                all: 'unset', cursor: 'pointer', height: 52, borderRadius: 16,
    ...
    <div style={{ height: '100%', overflowY: 'auto', overflowX: 'hidden', padding: '0 0 120px', background: 'var(--bg)' }}>
      <div style={{ padding: '6px 18px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <button onClick={onBack} style={{
          all: 'unset', cursor: 'pointer', width: 40, height: 40, borderRadius: 99, background: 'var(--paper)',
        ...
        <button onClick={() => onAddInCat(catId)} style={{
          all: 'unset', cursor: 'pointer', width: 40, height: 40, borderRadius: 99, background: cat.color,
        ...
        {cat.budget > 0 && (
          <div style={{ width: '70%', marginTop: 14 }}>
            ...
            <div style={{ fontFamily: 'var(--ui)', fontSize: 12, color: over ? 'var(--accent)' : 'var(--ink-soft)', marginTop: 8 }}>
              {over ? `${fmt(spent - cat.budget)} por encima del presupuesto` : `${fmt(cat.budget - spent)} libres de ${fmt(cat.budget)}`}
            </div>
          </div>
        )}
      </div>
