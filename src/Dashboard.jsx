...
import { CatIcon, Donut, ProgressBar, Glyph, GLYPHS } from './components'
import { fmt, fmtNum } from './data'

// Tarjeta de categoría reutilizable
function CatCard({ c, spent, onOpen }) {
    ...
    <button onClick={() => onOpen(c.id)} style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
          ...
          <span style={{ fontFamily: 'var(--display)', fontSize: 15, fontWeight: 600, color: over ? 'var(--accent)' : 'var(--ink)', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap', flexShrink: 0 }}>{fmt(spent)}</span>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <span style={{ fontFamily: 'var(--ui)', fontSize: 11.5, color: over ? 'var(--accent)' : 'var(--ink-soft)' }}>
            {c.budget > 0 ? over ? `+${fmt(spent - c.budget)}` : `${fmt(c.budget - spent)} libre` : 'sin presupuesto'}
          </span>
        </div>
      </div>
    </button>
...
function SectionHead({ title, hint, total }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', padding: '22px 24px 10px' }}>
      ...
      {total != null && <span style={{ fontFamily: 'var(--display)', fontSize: 16, fontWeight: 600, color: 'var(--ink)', fontVariantNumeric: 'tabular-nums' }}>{fmt(total)}</span>}
    </div>
  )
}

export default function Dashboard({ tx, cats, jornal, onOpenCategory, onScrollChange, onOpenSettings, onOpenAdd }) {
  ...
  const gastado = cats.filter((c) => c.type === 'gasto').reduce((s, c) => s + spentByCat[c.id], 0)
  const invertido = cats.filter((c) => c.type === 'inversion').reduce((s, c) => s + spentByCat[c.id], 0)
  const gastosFijos = cats.filter((c) => c.type === 'gasto' && c.flow === 'fijo').reduce((s, c) => s + spentByCat[c.id], 0)
  ...
  const gastoCats = cats.filter((c) => c.type === 'gasto' && spentByCat[c.id] > 0)
  ...
  const activeCats = cats.filter((c) => c.budget > 0 || spentByCat[c.id] > 0)
    .sort((a, b) => spentByCat[b.id] - spentByCat[a.id])
    ...
    <div ref={scrollRef} onScroll={(e) => onScrollChange && onScrollChange(e.target.scrollTop)}
      ...
      <div style={{ padding: '8px 22px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontFamily: 'var(--display)', fontSize: 21, fontWeight: 600, color: 'var(--ink)', letterSpacing: '-.01em', marginTop: 2 }}>Junio 2026</div>
        </div>
        <button onClick={onOpenSettings} style={{
          all: 'unset', cursor: 'pointer', width: 42, height: 42, borderRadius: 99, background: 'var(--ink)', color: '#F3EDE3',
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--display)', fontWeight: 600, fontSize: 16
        }}>FP</button>
      </div>

      {/* Hero card */}
      <div style={{
        margin: '0 18px', borderRadius: 30, background: 'var(--hero)', color: '#F4EFE6',
        ...
        <div style={{ position: 'relative' }}>
          ...
          <div style={{ fontFamily: 'var(--ui)', fontSize: 12.5, color: 'rgba(244,239,230,.5)', marginTop: 8 }}>
            de {fmt(jornal)} de jornal · {pct(libre)}% libre
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
            {[['Fijos', gastosFijos, 'var(--accent)'], ['Diarios', gastosDiarios, '#E0913C'], ['Inversión', invertido, '#7FB39A'], ['Ahorro', libre, 'rgba(244,239,230,.55)']].map(([l, v, col]) =>
              <div key={l} style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontFamily: 'var(--display)', fontSize: 14, fontWeight: 600, marginTop: 3, fontVariantNumeric: 'tabular-nums' }}>{fmtNum(v, 0)}€</div>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Proyección anual */}
      <div style={{
        margin: '14px 18px 0', borderRadius: 24, background: 'var(--paper)', padding: '16px 20px 18px',
        ...
        <div style={{ fontFamily: 'var(--ui)', fontSize: 11.5, color: 'var(--ink-soft)', marginTop: 12, lineHeight: 1.45 }}>
          Hucha de fin de año estimada: <span style={{ color: 'var(--ink)', fontWeight: 600 }}>{fmt(Math.max(0, (libre + invertido) * 12))}</span>
        </div>
      </div>

      {/* Donut card */}
      <div style={{
        margin: '16px 18px 0', borderRadius: 28, background: 'var(--paper)', padding: '20px 22px 22px',
        boxShadow: '0 1px 2px rgba(40,28,16,.05), 0 8px 24px -16px rgba(40,28,16,.18)'
      }}>
      ...
      {catsFijos.length === 0
        ...
        : <div style={{ margin: '0 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {catsFijos.map((c) => <CatCard key={c.id} c={c} spent={spentByCat[c.id]} onOpen={onOpenCategory} />)}
        </div>
      }
      ...
      {catsDiarios.length === 0
        ...
        : <div style={{ margin: '0 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {catsDiarios.map((c) => <CatCard key={c.id} c={c} spent={spentByCat[c.id]} onOpen={onOpenCategory} />)}
        </div>
      }
