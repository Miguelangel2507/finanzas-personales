...
import { fmt, ICONS } from './data'

const CAT_COLORS = ['#D85C32', '#E0913C', '#C99A4B', '#3E6B57', '#4E7E86', '#5F8A7A', '#2E7D5B', '#4E7CA8', '#7C6B9C', '#A85B5B', '#B0623F', '#C98A5B', '#8C9A86', '#6B6E78']

function slug(s) {
  return (s || 'cat').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '').slice(0, 16) || 'cat'
}

export default function CategoriesManager({ open, onClose, tx, cats, onAdd, onUpdate, onDelete }) {
  ...
  useEffect(() => {
    if (open) { setView('list'); setConfirmDel(false) }
  }, [open])
  ...
  const save = () => {
    ...
    const payload = {
      ...
      name,
      sub: (draft.sub || '').trim() || (draft.type === 'inversion' ? 'Inversión' : draft.flow === 'fijo' ? 'Gasto fijo' : 'Gasto diario'),
      budget,
      type: draft.type,
      color: draft.color,
      icon: draft.icon
          ...
          {view === 'list'
            ? <React.Fragment>
              <button onClick={startNew} style={{
                all: 'unset', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12, width: '100%', boxSizing: 'border-box',
              ...
              <div style={{ background: 'var(--paper)', borderRadius: 22, overflow: 'hidden', boxShadow: '0 1px 2px rgba(40,28,16,.04)' }}>
                {CATEGORIES.map((c, i) =>
                  <button key={c.id} onClick={() => startEdit(c)} style={{
                    all: 'unset', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 13, padding: '12px 15px',
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontFamily: 'var(--ui)', fontSize: 12, color: 'var(--ink-soft)', marginTop: 1 }}>
                        {c.type === 'inversion' ? 'Inversión' : c.flow === 'fijo' ? 'Gasto fijo' : 'Gasto diario'}{c.budget > 0 ? ` · ${fmt(c.budget)}/mes` : ' · sin presupuesto'}
                      </div>
                    </div>
            ...
            : draft && <React.Fragment>
              ...
              {/* nombre */}
              <label style={{ display: 'block', fontFamily: 'var(--ui)', fontSize: 12.5, fontWeight: 600, color: 'var(--ink-soft)', padding: '0 4px 7px' }}>Nombre</label>
              <input value={draft.name} onChange={(e) => D('name', e.target.value)} placeholder="p. ej. Viajes"
                style={{
                  width: '100%', boxSizing: 'border-box', border: 'none', outline: 'none', background: 'var(--paper)', borderRadius: 15, padding: '14px 16px',
              ...
              <div style={{ display: 'flex', background: '#00000008', borderRadius: 14, padding: 3 }}>
                {[['gasto', 'Gasto'], ['inversion', 'Inversión']].map(([k, l]) =>
                  <button key={k} onClick={() => D('type', k)} style={{
                    all: 'unset', cursor: 'pointer', flex: 1, textAlign: 'center', padding: '9px 0', borderRadius: 11,
                    fontFamily: 'var(--ui)', fontSize: 14, fontWeight: 600, color: draft.type === k ? 'var(--ink)' : 'var(--ink-soft)',
                    background: draft.type === k ? 'var(--paper)' : 'transparent', boxShadow: draft.type === k ? '0 1px 3px rgba(40,28,16,.12)' : 'none'
                  }}>{l}</button>
                )}
              </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 11, padding: '0 2px' }}>
                {CAT_COLORS.map((col) =>
                  <button key={col} onClick={() => D('color', col)} style={{
                    all: 'unset', cursor: 'pointer', width: 34, height: 34, borderRadius: 11, background: col,
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 9 }}>
                {iconKeys.map((k) => {
                  ...
                  return (
                    <button key={k} onClick={() => D('icon', ICONS[k])} style={{
                      all: 'unset', cursor: 'pointer', aspectRatio: '1', borderRadius: 13, display: 'flex', alignItems: 'center', justifyContent: 'center',
                ...
                <div style={{ marginTop: 26 }}>
                  {!confirmDel
                    ? <button onClick={() => setConfirmDel(true)} style={{
                      all: 'unset', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', boxSizing: 'border-box',
