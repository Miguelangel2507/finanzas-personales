// ─────────────────────────────────────────────────────────────
// Componentes compartidos: CatIcon, Donut, ProgressBar, Glyph, GLYPHS
// ─────────────────────────────────────────────────────────────
import React, { useState, useEffect } from 'react'

// Icono de categoría dentro de una baldosa redondeada con el color de la categoría
export function CatIcon({ cat, size = 44, radius = 14, iconScale = 0.55 }) {
...
export function Donut({ slices, size = 188, stroke = 26, gap = 0.018, children, animate = true }) {
...
export function ProgressBar({ value, max, color, track = '#00000010', height = 6 }) {
  const pct = Math.max(0, Math.min(1, max > 0 ? value / max : 0))
  return (
    <div style={{ height, borderRadius: 99, background: track, overflow: 'hidden', width: '100%' }}>
      <div style={{
        height: '100%', width: `${pct * 100}%`, background: color, borderRadius: 99,
...
export function Glyph({ d, size = 22, stroke = 2, color = 'currentColor' }) {
  return (
