...
import { fmt, fmtNum, HISTORY } from './data'

export default function History({ tx, jornal }) {
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', overflowX: 'hidden', padding: '0 0 120px' }}>
      ...
      {/* Gráfico */}
      <div style={{
        margin: '16px 18px 0', borderRadius: 28, background: 'var(--paper)', padding: '18px 18px 16px',
        ...
        <div style={{ display: 'flex', background: '#00000008', borderRadius: 13, padding: 3, marginBottom: 18 }}>
          {[['ahorro', 'Ahorro'], ['gastos', 'Gastos']].map(([k, l]) => (
            <button key={k} onClick={() => setMetric(k)} style={{
              ...
              boxShadow: metric === k ? '0 1px 3px rgba(40,28,16,.12)' : 'none', transition: 'all .2s'
            }}>{l}</button>
          ))}
        </div>
        ...
        <div style={{ position: 'relative', height: chartH, display: 'flex', alignItems: 'flex-end', gap: 8 }}>
          ...
          <div style={{ position: 'absolute', left: 0, right: 0, bottom: `${(avg / maxV) * chartH}px`, borderTop: '1.5px dashed #00000020', zIndex: 1 }}>
            <span style={{ position: 'absolute', right: 0, top: -16, fontFamily: 'var(--ui)', fontSize: 9.5, color: 'var(--ink-soft)', background: 'var(--paper)', padding: '0 4px' }}>media {fmtNum(avg, 0)}€</span>
          </div>
          {data.map((d, i) => {
            ...
            return (
              <button key={d.m} onClick={() => setSel(i)} style={{ all: 'unset', cursor: 'pointer', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7, height: '100%', justifyContent: 'flex-end' }}>
                <div style={{
                  width: '100%', maxWidth: 30, height: h, borderRadius: 9,
      ...
      <div style={{
        margin: '16px 18px 0', borderRadius: 28, background: 'var(--paper)', padding: '18px 22px 20px',
        boxShadow: '0 1px 2px rgba(40,28,16,.05), 0 8px 24px -16px rgba(40,28,16,.18)'
      }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
          {data.map(d => (
            <div key={d.m} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <span style={{ fontFamily: 'var(--display)', fontSize: 12, fontWeight: 600, color: 'var(--ink)', width: 46, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{fmtNum(d.ahorro, 0)}€</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, margin: '16px 18px 0' }}>
        {[['Mejor mes', bestMonth.m, fmt(bestMonth.ahorro)], ['Media mensual', '', fmt(Math.round(totalAhorro / data.length))]].map(([l, sub, v], i) => (
